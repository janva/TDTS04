...
public class SimpleCheck implements Checker
{
...

	{
	    ...
	    if (moreRight)
	    {
		if (((nextIncrementer + lastMove.getCol()) < board.getWidth()) && (board.getMarkAtPosition(
	    ...
	    if (moreLeft)
	    {
		if (((lastMove.getCol() - nextIncrementer) >= 0) && board.getMarkAtPosition(
			new Position(lastMove.getRow(), lastMove.getCol() - nextIncrementer)) == mark)
		{
		    if (++horizontalCount >= WIN_LIMIT)
		    {
		...
		if (lastMove.getRow() + nextIncrementer < board.getHeight()
			...
			&& board.getMarkAtPosition(new Position
				...
				lastMove.getCol() + nextIncrementer)) == mark)
		{
		    System.out.println("wincounter was " + diagonalUpperLeftToLowerRightCount);
		    System.out.println("location " + (lastMove.getRow() + nextIncrementer)+ ", "
